...
class FacialEmotionTrainer:
    ...
    def save_best_model(self):
        ...
        # Also save to main data/models directory
        main_model_dir = Path('../data/models')
        main_model_dir.mkdir(exist_ok=True, parents=True)
        torch.save(self.model.state_dict(), main_model_dir / 'facial_emotion_model.pth')


def main():
    parser = argparse.ArgumentParser(description='Train Facial Emotion Recognition Model')
    parser.add_argument('--data_path', type=str, required=True, help='Path to FER2013 dataset')
    parser.add_argument('--model_type', type=str, default='cnn', choices=['cnn', 'resnet', 'mobilenet'],
    ...
    parser.add_argument('--save_dir', type=str, default='checkpoints/facial', help='Directory to save models')
    ...
    print("="*60)
    print("MAITRI - Facial Emotion Recognition Training")
    print("="*60)
    ...
    # Load data
    print("\nLoading dataset...")
    train_loader, val_loader, test_loader = prepare_fer2013_dataloader(
        args.data_path, 
    ...
    model = get_model(model_type=args.model_type, num_classes=7)
    print(f"✓ Model parameters: {sum(p.numel() for p in model.parameters()):,}")
    
    # Create trainer
    trainer = FacialEmotionTrainer(model, device, save_dir=args.save_dir)
